...
class AuthServices {
  ...
  register(RegisterModel registerModel) async {
    try {
      final response = await dio.post(ConstantsURL.registerURL,
      ...
      Get.snackbar('Filed', 'you  are not accessed',
          snackPosition: SnackPosition.BOTTOM,
  ...
  login(String user_email, String user_pass) async {
    try {
      var data = {'user_email': user_email, 'user_pass': user_pass};
      final response = await dio.post('${ConstantsURL.loginURl}', data: data);
      authModel = AuthModel.fromJson(response.data);
      await CacheHelper.saveData(
          key: 'token', value: authModel.data.token.toString());
  ...
  logout() async {
    ...
    try {
      var response = await dio.post(
        '${ConstantsURL.logoutURl} ',
      );
      logOutModel = LogOutModel.fromJson(response.data);
      Get.snackbar('Success', '${logOutModel.message}',
          snackPosition: SnackPosition.BOTTOM, duration: Duration(seconds: 2));
      // Get.offAndToNamed(AppRoutes.home);
    } catch (e) {
      Get.snackbar('Filed', '${logOutModel.message}',
          snackPosition: SnackPosition.BOTTOM,
          duration: const Duration(seconds: 2));
      print(e);
    }
  ...
  forgetPassword(String email) async {
    try {
      final response = await dio.post('${ConstantsURL.passwordForgotURl}',
